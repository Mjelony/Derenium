...
package net.mcreator.derenium.world.biome;
...
import net.mcreator.derenium.block.PalmLogBlock;
import net.mcreator.derenium.block.PalmLeavesBlock;
...
public class PalmIslandBiome extends DereniumModElements.ModElement {
	@ObjectHolder("derenium:palm_island")
	public static final CustomBiome biome = null;
	public PalmIslandBiome(DereniumModElements instance) {
		super(instance, 45);
	}
	...
	public void init(FMLCommonSetupEvent event) {
		BiomeDictionary.addTypes(biome, BiomeDictionary.Type.BEACH, BiomeDictionary.Type.FOREST);
		...
		public CustomBiome() {
			...
			addFeature(GenerationStage.Decoration.VEGETAL_DECORATION,
					new CustomTreeFeature()
							.withConfiguration((new BaseTreeFeatureConfig.Builder(new SimpleBlockStateProvider(PalmLogBlock.block.getDefaultState()),
									new SimpleBlockStateProvider(PalmLeavesBlock.block.getDefaultState()))).baseHeight(7)
	...
	static class CustomTreeFeature extends AbstractTreeFeature<BaseTreeFeatureConfig> {
				...
				if (!spawnTree) {
					...
					if (position.getY() < world.getHeight() - height - 1) {
								...
								for (int i2 = position.getZ() - j1; i2 <= position.getZ() + j1; ++i2) {
									...
									if (Math.abs(position.getX()) != j1 || Math.abs(j2) != j1 || rand.nextInt(2) != 0 && i4 != 0) {
										...
										if (state.getBlock().isAir(state, world, blockpos) || state.getMaterial().blocksMovement()
												...
												|| state.getBlock() == PalmLeavesBlock.block.getDefaultState().getBlock()) {
											setTreeBlockState(changedBlocks, world, blockpos, PalmLeavesBlock.block.getDefaultState(), bbox);
										}
									}
								}
							}
						...
						for (int genh = 0; genh < height; genh++) {
							...
							setTreeBlockState(changedBlocks, world, genhPos, PalmLogBlock.block.getDefaultState(), bbox);
		...
		private boolean canGrowInto(Block blockType) {
			return blockType.getDefaultState().getMaterial() == Material.AIR || blockType == PalmLogBlock.block.getDefaultState().getBlock()
					|| blockType == PalmLeavesBlock.block.getDefaultState().getBlock() || blockType == Blocks.SAND.getDefaultState().getBlock()
